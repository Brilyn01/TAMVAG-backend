...
public class UserAuthService {
    ...
    public UserAuthService(
            ...
            @Value("${tamva.security.user-access-token-ttl-seconds:900}")
            long userAccessTokenTtlSeconds,
            @Value("${tamva.security.refresh-token-ttl-days:7}")
    ...
    public SignUpResponse signUp(SignUpRequest request) {
        ...
        Customer customer = new Customer(UUID.randomUUID(), normalizedEmail, "INDIVIDUAL", "ACTIVE");
    ...
    public SignInResponse signIn(SignInRequest request) {
        ...
        TamvaUser user = tamvaUserRepository.findByEmailIgnoreCase(normalizedEmail)
        ...
        if ("SUSPENDED".equalsIgnoreCase(user.getStatus()) || "CLOSED".equalsIgnoreCase(user.getStatus())) {
    ...
    public RefreshResponse refresh(RefreshRequest request) {
        ...
        RefreshTokenSession session = refreshTokenSessionRepository.findByTokenHash(hashedToken)
    ...
    @Transactional
    public LogoutResponse logout(String rawRefreshToken, UUID authenticatedUserId) {
        if (rawRefreshToken != null && !rawRefreshToken.isBlank()) {
            String hashedToken = hashToken(rawRefreshToken.trim());
            refreshTokenSessionRepository.findByTokenHash(hashedToken)
                    .ifPresent(session -> {
                        session.setRevoked(true);
                        refreshTokenSessionRepository.save(session);
                    });
        ...
        if (authenticatedUserId != null) {
            tamvaUserRepository.findById(authenticatedUserId).ifPresent(user -> {
                List<RefreshTokenSession> sessions = refreshTokenSessionRepository.findByUser(user);
    ...
    public UserMeResponse getMe(UUID userId) {
        TamvaUser user = tamvaUserRepository.findById(userId)
    ...
    @Transactional
    public UserMeResponse updateMe(UUID userId, UpdateMeRequest request) {
        TamvaUser user = tamvaUserRepository.findById(userId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "User not found"));
        ...
        TamvaUser saved = tamvaUserRepository.save(user);

        return mapToUserMeResponse(saved);
    }

    private UserMeResponse mapToUserMeResponse(TamvaUser user) {
    ...
    private String generateAccessToken(TamvaUser user) {
        Instant issuedAt = Instant.now();
        Instant expiresAt = issuedAt.plusSeconds(userAccessTokenTtlSeconds);
        ...
        JwtClaimsSet.Builder claimsBuilder = JwtClaimsSet.builder()
                ...
                .claim("scope", "profile:read cases:read cases:write")
                .claim("token_type", "user");

        if (user.getCustomerId() != null) {
            claimsBuilder.claim("customer_id", user.getCustomerId().toString());
        }
    ...
    private void createRefreshTokenSession(TamvaUser user, String rawRefreshToken) {
